...
import fr.syalioune.scheduling.quartz.util.ExpiringLeaderLatch;
...
public class ZkSemaphoreLeaderhsipRenewalThread implements Runnable {
  ...
  public void run() {
    try {
      LOG.debug("Starting semaphore leadership check loop for Quartz scheduler {} and NodeId {}", zkSemaphore.getSchedName(), zkSemaphore.getNodeId());
      ...
      if(LOG.isDebugEnabled()) {
        participants.forEach(participant -> {
          LOG.debug("{} - isLeader = {}", participant.getId(), participant.isLeader());
        });
      }
      ...
      LOG.debug("Ending semaphore leadership check loop for Quartz scheduler {} and NodeId {}", zkSemaphore.getSchedName(), zkSemaphore.getNodeId());
  ...
  private ExpiringLeaderLatch buildLatch() {
    ...
    if(zkSemaphore.getZookeeperAuthUsername() != null) {
      String digest = String.join(":", zkSemaphore.getZookeeperAuthUsername(), zkSemaphore.getZookeeperAuthPassword());
      builder.authorization(zkSemaphore.getZookeeperAuthScheme(), digest.getBytes());
    }
    CuratorFramework client = builder.build();
    if(zkSemaphore.getNodeId() == null) {
      zkSemaphore.setNodeId(UUID.randomUUID().toString());
    }
